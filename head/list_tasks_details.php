...
<div id="content" class="p-4 p-md-5 pt-5">
    <div id="wrapper">
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">List of Deployed Task Details</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                                    ...
                                    <tbody id="tbody">
                                        ...
                                        if (mysqli_num_rows($getactivesection)>0) {
                                            while ($row = $getactivesection->fetch_assoc()) {
												$task_sec = $row['sec_id'];
												$result = mysqli_query($con,"SELECT COUNT('id') as sec_total_tasks FROM tasks_details LEFT JOIN accounts ON accounts.username = tasks_details.in_charge LEFT JOIN task_list ON tasks_details.task_name = task_list.task_name LEFT JOIN task_class ON task_list.task_class = task_class.id WHERE tasks_details.task_status = 1 AND tasks_details.task_for = '$task_sec'");
												...
												<tr>
													<td>".$row['sec_name']."</td>
													<td>".$row['dept_name']."</td>
													<td><a href='task_details.php?section=$task_sec'> <button class='btn btn-md btn-folder' style='margin-left: 10px'><i class='fa fa-folder'></i> ".$total_task." Deployed Tasks</button></a></td>
												</tr>
												";
                                            } 
                                        }
