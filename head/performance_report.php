  ...
  while ($row = $result->fetch_assoc()) {
    ...
    if ($total_task=='0') {
      ...
      if ($formatted_number != 0){
        ...
        elseif ($formatted_number >= 2.80){
          $propotion = ($formatted_num - 2.80) / (2.99 - 2.80);
          $percentage = 105 + ($propotion * 14);
          $equivalent = number_format($percentage, 2);
        }
        elseif ($formatted_number >= 2.70){
          $propotion = ($formatted_num - 2.70) / (2.79 - 2.70);
          $percentage = 95 + ($propotion * 9);
          $equivalent = number_format($percentage, 2);
        }
        elseif ($formatted_number >= 2.60){
          $propotion = ($formatted_num - 2.60) / (2.69 - 2.60);
          $percentage = 80 + ($propotion * 9);
          $equivalent = number_format($percentage, 2);
        }
        elseif ($formatted_number >= 2.59){
          $propotion = ($formatted_num - 2.50) / (2.59 - 2.50);
          $percentage = 75 + ($propotion * 9);
          $equivalent = number_format($percentage, 2);
        }
        elseif ($formatted_num <= 2.49) {
          $equivalent = number_format(70, 2);
        }
      }
      else {
        $equivalent = 0.00;
