...
if (isset($_POST['calculate'])) {
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    if (isset($_POST['date_to']) && isset($_POST['date_from'])) {
      ...
      $task_query = "SELECT *, (SELECT COUNT(id) FROM tasks_details WHERE in_charge = '$assignee' AND task_status = 1 AND DATE(due_date) >= '$date_from' AND DATE(due_date) <= '$date_to' AND task_class NOT IN (5, 6)) AS task_total, (SELECT COUNT(id) FROM tasks_details WHERE in_charge = '$assignee' AND task_status = 1 AND DATE(due_date) >= '$date_from' AND DATE(due_date) <= '$date_to' AND task_class = 6) AS report_total FROM tasks_details WHERE task_status = 1 AND DATE(due_date) >= '$date_from' AND DATE(due_date) <= '$date_to' AND in_charge = '$assignee'";
    } else {
