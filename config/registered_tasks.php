...
if (isset($_POST['editTask'])) {
  $getDetails = mysqli_fetch_assoc(mysqli_query($con, "SELECT t.task_id, tl.task_name, tl.task_details, tl.task_for, t.submission, t.requirement_status, tl.task_class, GROUP_CONCAT(in_charge SEPARATOR ', ') AS in_charge_list FROM tasks t JOIN task_list tl ON t.task_id=tl.id JOIN task_class tc ON tl.task_class=tc.id WHERE tl.id='{$_POST['id']}' GROUP BY t.task_id"));
...
if (isset($_POST['updateTask'])) {
  ...
  if ($taskName !== '' && $taskDetails !== '' && $submission !== '') {
