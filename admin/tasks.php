								...
								<div class="table-responsive">
									<table width="100%" class="table table-striped table-hover" id="table_task">
										...
										<tbody id="show_task">
											<?php
												$con->next_result();
												$result = mysqli_query($con,"SELECT * FROM tasks_details JOIN accounts ON tasks_details.in_charge=accounts.username JOIN task_class on task_class.id=tasks_details.task_class WHERE tasks_details.task_status=1 AND tasks_details.status='$status'");               
												if (mysqli_num_rows($result)>0) { 
													while ($row = $result->fetch_assoc()) {
														...
														if ($status == "FINISHED"){
															...
															<td class='".$class."'><center />" . $date_accomplished . "</td>
															...
															<td class='".$class."'><center />" . $row["task_class"] . "</td> 
															<td class='".$class."' style='text-align: justify'> <img src=".$imageURL." title=".$row["username"]." style='width: 50px;height: 50px; border-radius: 50%; object-fit: cover; margin-right: 15px; margin-left: 0'>" . $emp_name . "</td>  
															<td class='".$class."'><center />" . $due_date . "</td> 
															<td class='".$class."'><center /><p class='label label-".$class_label."' style='font-size:100%;'>".$sign."</p></td>
															</tr>";
														}
													}
