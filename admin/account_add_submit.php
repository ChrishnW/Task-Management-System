...
<?php
    ...
    $con->next_result();   
    $query = "INSERT INTO accounts (card,username,password,fname,lname,email,access,sec_id,status)VALUES((('$card')),(UPPER('$username')),('$hash'),(UPPER('$fname')),(UPPER('$lname')),(LOWER('$email')),(('$access')),(UPPER('$section')),(('1')))";
    $result = mysqli_query($con, $query) or die('Error querying database.');
    $con->next_result(); 

    $systemlog = "INSERT INTO system_log (action, date_created, user) VALUES ('$username account registered.', '$systemtime', 'ADMIN')";
